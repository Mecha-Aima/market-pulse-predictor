...
def test_ci_workflow_file_exists(ci_workflow_path):
    ...
    assert ci_workflow_path.exists(), f"CI workflow file should exist at {ci_workflow_path}"


def test_workflow_has_name(ci_workflow_content):
    """Test: Workflow has a descriptive name."""
    assert "name" in ci_workflow_content, "Workflow should have a name"
    assert len(ci_workflow_content["name"]) > 0, "Workflow name should not be empty"
...
def test_workflow_triggers_on_pull_request(ci_workflow_content):
    ...
    if isinstance(pr_config, dict):
        ...
        assert "main" in pr_config["branches"], "Pull request trigger should include main branch"


def test_workflow_has_lint_job(ci_workflow_content):
    """Test: Workflow has lint job."""
    assert "jobs" in ci_workflow_content, "Workflow should have jobs"
    assert "lint" in ci_workflow_content["jobs"], "Workflow should have a lint job"


def test_workflow_has_test_job(ci_workflow_content):
    """Test: Workflow has test job."""
    assert "test" in ci_workflow_content["jobs"], "Workflow should have a test job"
...
def test_lint_job_uses_python_311(ci_workflow_content):
    ...
    assert len(python_steps) > 0, "Lint job should use actions/setup-python"

    python_step = python_steps[0]
    python_version = python_step.get("with", {}).get("python-version", "")

    assert "3.11" in str(python_version), f"Lint job should use Python 3.11, got {python_version}"
...
def test_test_job_uses_python_311(ci_workflow_content):
    ...
    assert len(python_steps) > 0, "Test job should use actions/setup-python"

    python_step = python_steps[0]
    python_version = python_step.get("with", {}).get("python-version", "")

    assert "3.11" in str(python_version), f"Test job should use Python 3.11, got {python_version}"
...
def test_lint_job_runs_ruff(ci_workflow_content):
    ...
    assert len(ruff_commands) > 0, "Lint job should run ruff"
...
def test_workflow_caches_dependencies(ci_workflow_content):
    ...
    assert has_cache, "Workflow should cache pip dependencies for faster builds"
...
def test_test_job_has_coverage_reporting(ci_workflow_content):
    ...
    assert len(coverage_commands) > 0, "Test job should include coverage reporting"
...
def test_workflow_sets_pythonpath(ci_workflow_content):
    ...
    assert has_pythonpath, "Test job should set PYTHONPATH=. for imports to work"
...
def test_workflow_installs_dependencies(ci_workflow_content):
    ...
    for _job_name, job_config in jobs.items():
        ...
        run_commands = [s.get("run", "") for s in steps if "run" in s]
        install_commands = [
            cmd for cmd in run_commands if "pip install" in cmd and "requirements.txt" in cmd
        ]
        if install_commands:
            has_install = True
            break

    assert has_install, "Workflow should install dependencies from requirements.txt"
...
def test_workflow_downloads_nltk_data(ci_workflow_content):
    ...
    run_commands = [s.get("run", "") for s in steps if "run" in s]
    nltk_commands = [
        cmd for cmd in run_commands if "nltk" in cmd.lower() and "download" in cmd.lower()
    ]

    assert len(nltk_commands) > 0, "Test job should download NLTK vader_lexicon data"
...
def test_lint_job_runs_on_ubuntu(ci_workflow_content):
    ...
    assert "ubuntu" in runs_on.lower(), f"Lint job should run on Ubuntu, got {runs_on}"
...
def test_test_job_runs_on_ubuntu(ci_workflow_content):
    ...
    assert "ubuntu" in runs_on.lower(), f"Test job should run on Ubuntu, got {runs_on}"
