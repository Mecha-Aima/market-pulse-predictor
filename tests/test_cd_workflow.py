...
def test_cd_workflow_file_exists(cd_workflow_path):
    ...
    assert cd_workflow_path.exists(), \
        f"CD workflow file should exist at {cd_workflow_path}"
...
def test_workflow_has_build_and_push_job(cd_workflow_content):
    ...
    assert 'jobs' in cd_workflow_content, \
...
def test_workflow_has_deploy_job(cd_workflow_content):
    ...
    assert 'deploy' in cd_workflow_content['jobs'], \
...
def test_deploy_job_depends_on_build(cd_workflow_content):
    ...
    assert 'needs' in deploy_job, \
        "Deploy job should have dependencies"
    ...
    assert 'build-and-push' in needs, \
        "Deploy job should depend on build-and-push job"
...
def test_workflow_uses_aws_credentials_secret(cd_workflow_content):
    ...
    assert has_aws_key, \
        "Workflow should use AWS_ACCESS_KEY_ID secret"
...
def test_workflow_uses_aws_secret_access_key_secret(cd_workflow_content):
    ...
    assert has_aws_secret, \
        "Workflow should use AWS_SECRET_ACCESS_KEY secret"
...
def test_workflow_uses_ecr_repository_secret(cd_workflow_content):
    ...
    assert has_ecr, \
        "Workflow should reference ECR repository for Docker image push"
...
def test_workflow_uses_ec2_ssh_key_secret(cd_workflow_content):
    ...
    assert has_ssh_key, \
        "Deploy job should use EC2_SSH_KEY secret for SSH access"
...
def test_build_job_pushes_to_ecr(cd_workflow_content):
    ...
    has_ecr_login = any('ecr' in cmd.lower() for cmd in uses_commands) or \
                    any('ecr' in cmd.lower() and 'login' in cmd.lower() for cmd in run_commands)
...
def test_deploy_job_ssh_to_ec2(cd_workflow_content):
    ...
    has_ssh = any('ssh' in cmd.lower() for cmd in run_commands) or \
              any('ssh' in cmd.lower() for cmd in uses_commands)
    
    assert has_ssh, \
        "Deploy job should SSH into EC2 instance"
...
def test_deploy_job_pulls_docker_images(cd_workflow_content):
    ...
    assert has_docker_pull, \
        "Deploy job should pull Docker images on EC2"
...
def test_deploy_job_has_health_check(cd_workflow_content):
    ...
    assert has_health_check, \
        "Deploy job should verify health check after deployment"
...
def test_deploy_job_has_rollback_logic(cd_workflow_content):
    ...
    assert has_rollback, \
        "Deploy job should have rollback logic on failure"
